...
mazeGenerateBtn.addEventListener('click', startMazeMakingProcess);
...
Node.prototype.makeClassEl = function (txt) {
  this.className += ' ' + txt
}

//toggle loading wheel
function toggleLoadWheel() {
  console.log("hit");
  if (document.getElementsByClassName("loader")[0].style.display == "block") {
    document.getElementsByClassName("loader")[0].style.display = "";
  } else {
    document.getElementsByClassName("loader")[0].style.display = "block";
  }
}
//fucntion so that the loading message shows for large mazes
function startMazeMakingProcess() {
  toggleLoadWheel();
  setTimeout(make_maze, 50);
  setTimeout(toggleLoadWheel, 50);
}
    ...
    for (var h_loop = 1; h_loop <= h; h_loop++) {
      for (var w_loop = 1; w_loop <= w; w_loop++) {
        tblMaze.getChild(h_loop).getChild(w_loop).neighbors = [
          tblMaze.getChild(h_loop + 1).getChild(w_loop), //s
          tblMaze.getChild(h_loop).getChild(w_loop + 1), //e
          tblMaze.getChild(h_loop).getChild(w_loop - 1), //w
          tblMaze.getChild(h_loop - 1).getChild(w_loop) //n
        ];
      }
    }
